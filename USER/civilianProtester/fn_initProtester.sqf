 ...
 *
 *  When any protester turns hostile the whole protest breaks: every other
 *  protester flees, and any that carry a weapon draw and go hostile too.
 *
 *  Call from the unit's init field (runs everywhere, sets up on server):
 *      [this] call GRAD_civilianProtester_fnc_initProtester;
 *
 *  Optional second argument: weights array [flee, hostile]
 *      [this, [40, 20]] call GRAD_civilianProtester_fnc_initProtester;
 *
 *  Parameters:
 *      0: OBJECT  - the civilian unit
 *      1: ARRAY   - (optional) outcome weights [flee, hostile], default [40,20]
 */
...
// Kick off the crowd chant loop (random Rus1..Rus20 shouts ~every 10s, stops
// automatically once the unit leaves the "protesting" state).
[_unit] call GRAD_civilianProtester_fnc_chantLoop;
